import {
  ...
  legalAuthorities as iraqiLegalAuthorities,
  laws as iraqiLaws,
...
export const cases = [
  ...iraqiCases.map((caseItem) => ({
    ...caseItem,
    countryCode: "IQ",
    topicId: caseItem.topic.includes("إزالة الشيوع") || caseItem.topic.includes("ازالة الشيوع")
      ? "coownership-dissolution"
      : caseItem.topic.includes("التسجيل")
        ? "inheritance-registration"
        : caseItem.topic.includes("قسام") || caseItem.topic.includes("أحوال شخصية")
...
export const legalAuthorities = iraqiLegalAuthorities.map((authority) => ({
  ...authority,
  countryCode: "IQ"
}));
...
export const projectMetadata = {
  ...
  corpusNotice: "Global architecture pilot: Iraq includes source-linked civil and criminal procedural-route filters, filing checklists, an article 305 workflow, selected official co-ownership judgments, and a fully anonymized cross-border heirship demonstration; France and Egypt remain catalog pilots.",
