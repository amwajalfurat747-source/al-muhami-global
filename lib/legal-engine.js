...
function articleProvenance(article, law) {
  const primarySourceIds = article.primarySourceIds ?? law.sourceIds ?? [];
  return {
    recordType: "legal-text",
    verificationStatus: article.verificationStatus ?? "editorial-review-required",
    verificationTier: article.verificationTier ?? "source-linked",
    sourceCheckedOn: article.sourceCheckedOn ?? null,
    consolidationStatus: article.consolidationStatus ?? "not-recorded",
    effectiveDateStatus: article.effectiveDateStatus ?? "verify-at-relevant-date",
    editorialNotes: article.editorialNotes ?? [],
    primarySources: sourceFor(primarySourceIds)
  };
}
...
function authoritySearchText(authority) {
  return [
    authority.title,
    authority.number,
    authority.date,
...
export function getCatalog({ country = "العراق", countryCode, category, language = "en" } = {}) {
  ...
  return {
    ...
    stats: {
      ...
      officialGuidanceReferences: legalAuthorities.filter((item) => item.countryCode === selectedCountryCode).length,
...
export function getLawArticle({ lawId, articleNumber, countryCode, language = "en" } = {}) {
  ...
  const relatedAuthorities = legalAuthorities.filter((authority) => authority.lawArticles.includes(articleRef));
  ...
  return {
    ...
    sources: sourceFor(law.sourceIds),
    provenance: articleProvenance(article, law),
    ...
    relatedDecisions: relatedDecisions.map((decision) => ({
      ...
      documentAssets: decision.documentAssets ?? [],
      sources: sourceFor(decision.sourceIds)
    })),
    relatedAuthorities: relatedAuthorities.map((authority) => ({
      id: authority.id,
      title: authority.title,
      number: authority.number,
      date: authority.date,
      publicationType: authority.publicationType,
      issuer: authority.issuer,
      summary: authority.summary,
      verification: authority.verification,
      sourceIds: authority.sourceIds,
      documentAssets: authority.documentAssets ?? [],
      sources: sourceFor(authority.sourceIds)
    })),
...
export function searchLegalMaterials({
  ...
  for (const authority of legalAuthorities) {
    if (countryCode && authority.countryCode !== countryCode) continue;
    if (lawId && !authority.lawArticles.some((ref) => ref.startsWith(`${lawId}:`))) continue;
    if (articleNumber && !authority.lawArticles.some((ref) => ref.endsWith(`:${articleNumber}`))) continue;
    const authorityYear = Number(authority.date.slice(0, 4));
    if (yearFrom && authorityYear < Number(yearFrom)) continue;
    if (yearTo && authorityYear > Number(yearTo)) continue;
    const score = textScore(query, authoritySearchText(authority));
    if (query && score < 0.18) continue;
    results.push({
      type: "authority",
      id: authority.id,
      ...
      title: authority.title,
      subtitle: `${authority.issuer} — ${authority.number} — ${authority.date}`,
      snippet: makeSnippet(authority.summary),
      score: Math.min(1, score + 0.05),
      badges: ["توجيه قضائي رسمي", "ليس حكماً قضائياً"],
      sourceUrl: sourceFor(authority.sourceIds)[0]?.url ?? "",
      isSynthetic: false
    });
  }
...
export function getLegalSource(id, { language = "en" } = {}) {
  ...
  const authority = legalAuthorities.find((item) => item.id === id);
  if (authority) {
    return {
      type: "authority",
      item: { ...authority, sources: sourceFor(authority.sourceIds) }
    };
  }
    ...
    if (lawRecord && article) {
      ...
      return {
        ...
        item: {
          ...
          provenance: articleProvenance(article, lawRecord)
        }
      };
    }
  }
...
export const corpus = { languages, countries, laws, cases, decisions, legalAuthorities, sources, proceduralRoutes, filingTemplates, projectMetadata };
